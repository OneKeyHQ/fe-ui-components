...
export class OneKeyInjectedConnector extends AbstractConnector {
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.onekey.send as Send)('eth_accounts').then(sendReturn => parseSendReturn(sendReturn)[0])
    } catch {
